...
# Notes:
# SMTP Configuration: For Gmail, use smtp.gmail.com and port 587. You must use an App Password if 2FA is enabled.
# To generate an App Password:
# 1. Enable 2FA on your Google Account (https://myaccount.google.com/security).
# 2. Go to "2-Step Verification" > "App Passwords".
# 3. Generate a password for "Mail" and use it in the Sender Password field.
# Security: Password is encrypted with Fernet and stored in password.enc with key in key.key.
...
class PasswordManager:
    def __init__(self):
        ...
        self.fernet = None
        self.default_password = "InitialPass123!"  # Default password (change this as needed)
    ...
    def load_or_generate_key(self):
        if os.path.exists(self.key_file):
            ...
            with open(self.key_file, "wb") as key_file:
                key_file.write(key)
            # Set default password if no password file exists
            if not os.path.exists(self.password_file):
                self.save_password(self.default_password)
...
class LoginWindow:
    ...
    def setup_initial_password(self):
        ...
        initial_window.title("Set Initial Password")
        initial_window.geometry("300x200")
        initial_window.resizable(False, False)

        tk.Label(initial_window, text="Set Initial Password", font=("Arial", 14)).pack(pady=20)
        password_var = tk.StringVar()
        tk.Entry(initial_window, textvariable=password_var, show="*").pack(pady=10)
        tk.Button(initial_window, text="Set Password", command=lambda: self.save_initial_password(initial_window, password_var.get())).pack(pady=5)
...
class FileCryptoApp:
    def __init__(self, root):
        self.root = root
        self.root.title("File Encryption & Email Sender")
        self.root.geometry("1200x500")  # Initial window size
        ...
        # Create a horizontal PanedWindow to split the window into four sections
        self.pane = tk.PanedWindow(root, orient=tk.HORIZONTAL, sashrelief="raised", sashwidth=5)
        self.pane.pack(fill="both", expand=True)

        # Calculate initial equal width for each section (1200 / 4 = 300 pixels)
        initial_section_width = 300

        # Section 1: Encryption Section (Leftmost)
        ...
        # Section 2: Email Configuration Section
        self.email_config_frame = tk.Frame(self.pane)
        self.pane.add(self.email_config_frame, width=initial_section_width)

        tk.Label(self.email_config_frame, text="Email Configuration", font=("Arial", 14)).pack(pady=10)
        
        tk.Label(self.email_config_frame, text="Sender Email:").pack()
        self.sender_email_entry = tk.Entry(self.email_config_frame, width=40)
        self.sender_email_entry.pack()

        tk.Label(self.email_config_frame, text="Sender Password:").pack()
        self.sender_password_entry = tk.Entry(self.email_config_frame, width=40, show="*")
        self.sender_password_entry.pack()

        tk.Label(self.email_config_frame, text="SMTP Server:").pack()
        self.smtp_server_entry = tk.Entry(self.email_config_frame, width=40)
        self.smtp_server_entry.pack()

        tk.Label(self.email_config_frame, text="SMTP Port:").pack()
        self.smtp_port_entry = tk.Entry(self.email_config_frame, width=40)
        self.smtp_port_entry.pack()

        tk.Button(self.email_config_frame, text="Save Email Config", command=self.save_email_config).pack(pady=10)

        # Section 3: Email Section
        self.email_frame = tk.Frame(self.pane)
        self.pane.add(self.email_frame, width=initial_section_width)

        tk.Label(self.email_frame, text="Email Section", font=("Arial", 14)).pack(pady=10)
        
        tk.Label(self.email_frame, text="Recipient Email:").pack()
        self.email_entry = tk.Entry(self.email_frame, width=40)
        self.email_entry.pack()

        tk.Button(self.email_frame, text="Send Encrypted File & Key", command=self.send_email).pack(pady=5)

        # Section 4: Decryption Section (Rightmost)
    ...
    def save_email_config(self):
        ...
        try:
            self.smtp_port = int(self.smtp_port)  # Ensure port is an integer
        except ValueError:
    ...
    def send_email(self):
        if not self.email_configured:
            messagebox.showerror("Error", "Please save email configuration first")
            return

        if not self.encrypted_file or not self.private_key:
        ...
        msg["Subject"] = "Encrypted File and Private Key"

        body = "Attached are the encrypted file and private key."
        msg.attach(MIMEText(body, "plain"))

        part = MIMEBase("application", "octet-stream")
        part.set_payload(self.encrypted_file)
        encoders.encode_base64(part)
        part.add_header("Content-Disposition", "attachment", filename="encrypted_file.enc")
        msg.attach(part)
        ...
        key_part.set_payload(private_key_pem)
        encoders.encode_base64(key_part)
        key_part.add_header("Content-Disposition", "attachment", filename="private_key.pem")
        msg.attach(key_part)
